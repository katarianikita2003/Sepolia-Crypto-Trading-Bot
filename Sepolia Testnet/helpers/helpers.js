    ...
    try {
        ...
        const flashLoanParams = {
            ...
            params: ethers.AbiCoder.defaultAbiCoder().encode(
                ...
                [
                    arbitrageParams.buyExchange === 'uniswap' ? initialization.uniswap.router.target : initialization.sushiswap.router.target,
                    arbitrageParams.sellExchange === 'uniswap' ? initialization.uniswap.router.target : initialization.sushiswap.router.target,
...
const getPoolContract = async (exchange, token0Address, token1Address, poolFee, provider) => {
    try {
        console.log(`[🔍 POOL] Getting ${exchange.name} pool contract...`);
        console.log(`[📋 POOL] Token0: ${token0Address}`);
        console.log(`[📋 POOL] Token1: ${token1Address}`);
        console.log(`[📋 POOL] Fee: ${poolFee}`);

        // Get factory contract
        const factory = exchange.factory;

        // Check if factory is properly instantiated
        if (!factory) {
            throw new Error(`${exchange.name} factory not initialized`);
        }

        // Check if factory has the expected interface
        console.log(`[🔍 FACTORY] Checking factory deployment at: ${await factory.getAddress()}`);

        // Try to get the pool address with proper error handling
        let poolAddress;
        try {
            // For Uniswap V3, the function is getPool(token0, token1, fee)
            poolAddress = await factory.getPool(token0Address, token1Address, poolFee);
        } catch (factoryError) {
            console.error(`[❌ ERROR] Factory.getPool failed:`, factoryError.message);

            // Alternative approach - try with different method names
            try {
                // Some factories might use different function names
                if (typeof factory.poolFor === 'function') {
                    poolAddress = await factory.poolFor(token0Address, token1Address, poolFee);
                } else if (typeof factory.getPoolAddress === 'function') {
                    poolAddress = await factory.getPoolAddress(token0Address, token1Address, poolFee);
                } else {
                    throw new Error(`Factory contract doesn't have getPool, poolFor, or getPoolAddress function`);
                }
            } catch (altError) {
                throw new Error(`All factory method attempts failed: ${altError.message}`);
            }
        }

        console.log(`[📍 POOL] Pool address: ${poolAddress}`);

        // Check if pool exists (not zero address)
        if (poolAddress === "0x0000000000000000000000000000000000000000") {
            throw new Error(`Pool does not exist for ${token0Address}/${token1Address} with fee ${poolFee}`);
        }
        ...
        // Validate pool contract by checking basic properties
        try {
            const [poolToken0, poolToken1, poolFeeCheck] = await Promise.all([
                poolContract.token0(),
                poolContract.token1(),
                poolContract.fee()
            ]);

            console.log(`[✅ POOL] Pool validated:`);
            console.log(`[📋 POOL] Token0: ${poolToken0}`);
            console.log(`[📋 POOL] Token1: ${poolToken1}`);
            console.log(`[📋 POOL] Fee: ${poolFeeCheck}`);

            // Verify tokens match (they might be in different order)
            const tokens = [token0Address.toLowerCase(), token1Address.toLowerCase()];
            const poolTokens = [poolToken0.toLowerCase(), poolToken1.toLowerCase()];

            if (!tokens.every(token => poolTokens.includes(token))) {
                throw new Error(`Pool token mismatch. Expected: ${tokens.join(', ')}, Got: ${poolTokens.join(', ')}`);
            }

            if (Number(poolFeeCheck) !== poolFee) {
                throw new Error(`Pool fee mismatch. Expected: ${poolFee}, Got: ${poolFeeCheck}`);
            }

        } catch (validationError) {
            throw new Error(`Pool validation failed: ${validationError.message}`);
        }

        console.log(`[✅ SUCCESS] ${exchange.name} pool contract created successfully`);
        return poolContract;

    } catch (error) {
        console.error(`[❌ ERROR] Failed to get ${exchange.name} pool contract:`, error.message);
        throw error;
    }
};

// Alternative function to manually compute pool address (for Uniswap V3)
const computePoolAddress = (factory, token0, token1, fee) => {
    // This is a backup method if factory.getPool doesn't work
    // Note: This requires the CREATE2 computation which is complex
    // For now, we'll rely on the factory method
    throw new Error("Manual pool address computation not implemented yet");
};

// Enhanced factory validation function
const validateFactory = async (factory, exchangeName) => {
    try {
        console.log(`[🔍 VALIDATION] Validating ${exchangeName} factory...`);

        // Check if factory is deployed
        const factoryAddress = await factory.getAddress();
        console.log(`[✅ DEPLOYED] ${exchangeName} Factory is deployed at: ${factoryAddress}`);

        // Check if it has the required functions
        const hasGetPool = typeof factory.getPool === 'function';
        const hasPoolFor = typeof factory.poolFor === 'function';
        const hasGetPoolAddress = typeof factory.getPoolAddress === 'function';

        console.log(`[🔍 METHODS] Available methods:`);
        console.log(`  - getPool: ${hasGetPool}`);
        console.log(`  - poolFor: ${hasPoolFor}`);
        console.log(`  - getPoolAddress: ${hasGetPoolAddress}`);

        if (!hasGetPool && !hasPoolFor && !hasGetPoolAddress) {
            throw new Error(`Factory doesn't have any supported pool lookup methods`);
        }

        return true;

    } catch (error) {
        console.error(`[❌ ERROR] Factory validation failed:`, error.message);
        throw error;
    }
};

async function calculatePrice(poolContract, token0, token1, exchangeName) {
    // Example for Uniswap V3 pool
...
function calculateDifference(priceA, priceB) {
    const difference = Math.abs(priceA - priceB);
    const average = (priceA + priceB) / 2;
    return (difference / average) * 100; // Returns percentage difference
}
